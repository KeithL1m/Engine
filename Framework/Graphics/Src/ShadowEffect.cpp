...
{
    const KMath::Matrix4 matWorld = renderObject.transform.GetMatrix4();
    const KMath::Matrix4 matView = mLightCamera.GetViewMatrix();
    const KMath::Matrix4 matProj = mLightCamera.GetProjectionMatrix();

    TransformData data;
    data.wvp = KMath::Transpose(matWorld * matView * matProj);
...
void ShadowEffect::SetFocus(const KMath::Vector3& focusPosition)
{
...
void ShadowEffect::UpdateLightCamera()
{
    ...
    const KMath::Vector3& direction = mDirectionalLight->direction;
