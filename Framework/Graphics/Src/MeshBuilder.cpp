...
MeshPX MeshBuilder::CreateSkySpherePX(int slices, int rings, float radius)
{
    ...
    float vertRotation = (KMath::Constants::Pi / static_cast<float>(rings - 1));
    float horzRotation = (KMath::Constants::TwoPi / static_cast<float>(slices));
...
Mesh MeshBuilder::CreateSphere(int slices, int rings, float radius)
{
    ...
    for (int r = 0; r <= rings; r++)
    {
        ...
        for (int s = 0; s <= slices; s++)
        {
            ...
            KMath::Vector3 position = {x,y,z};
            KMath::Vector3 normal = KMath::Normalize(position);
            KMath::Vector3 tangent = KMath::Normalize({-z, 0.0f, x});
            KMath::Vector2 uvCoord = {u , v};
...
MeshPC MeshBuilder::CreateCylinderPC(int slices, int rings)
{
    ...
    for (int r = 0; r <= rings; ++r)
    {
        ...
        {
            float slice = static_cast<float>(s);
            float rotation = (slice / static_cast<float>(slices)) * KMath::Constants::TwoPi;
...
Mesh MeshBuilder::CreateSpriteQuad(float width, float height)
{
    ...
    mesh.vertices.push_back({{ -hw, -hh, 0.0f }, -KMath::Vector3::ZAxis, KMath::Vector3::XAxis, { 0.0f, 1.0f }});
    mesh.vertices.push_back({{ -hw, hh, 0.0f }, -KMath::Vector3::ZAxis, KMath::Vector3::XAxis, { 0.0f, 0.0f }});
    mesh.vertices.push_back({{ hw, hh, 0.0f }, -KMath::Vector3::ZAxis, KMath::Vector3::XAxis, { 1.0f, 0.0f }});
    mesh.vertices.push_back({{ hw, -hh, 0.0f }, -KMath::Vector3::ZAxis, KMath::Vector3::XAxis, { 1.0f, 1.0f }});
