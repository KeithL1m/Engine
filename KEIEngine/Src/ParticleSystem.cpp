...
void ParticleSystem::DebugUI()
{
	if (ImGui::CollapsingHeader("ParticleSystem", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		{
			mInfo.spawnDirection = KMath::Normalize(mInfo.spawnDirection);
		}
		...
		ImGui::DragFloat("MinLifeTime", &mInfo.maxLifeTime, 0.1f, mInfo.minLifeTime, 10.0f);
...
void ParticleSystem::SpawnParticle()
{
	...
	// gets float from 0-1
	KMath::Vector3 spawnDirection = mInfo.spawnDirection;
	if (mInfo.maxSpawnAngle > 0.0f)
	{
		...
		if (abs(KMath::Dot(mInfo.spawnDirection, Vector3::YAxis)) > 0.99f)
		{
