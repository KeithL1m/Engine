...
std::string FindTexture(const aiScene* scene, const aiMaterial* aiMaterial, aiTextureType textureType,
	const Arguments& args, const std::string& suffix,
...
Bone* BuildSkeleton(const aiNode& sceneNode, Bone* parent, Skeleton& skeleton, BoneIndexLookup& boneIndexLookup)
{
	...
	if (boneName.empty())
	{
		boneName = "NoName" + std::to_string(skeleton.bones.size());
	}
...
{
	const auto arg0pt = ParseArgs(argc, argv);
	if (!arg0pt.has_value())
	{
	...
	const Arguments& arguments = arg0pt.value();
	...
	if (scene->HasMeshes())
	{
		...
		for (uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex)
		{
			...
			const aiVector3D* textCoords = aiMesh->HasTextureCoords(0) ? aiMesh->mTextureCoords[0] : nullptr;
			for (uint32_t v = 0; v < numVertices; ++v)
			{
				...
				if (textCoords)
				{
					vertex.uvCoord = { textCoords[v].x, textCoords[v].y};
				}
			}
